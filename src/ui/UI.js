...
export class UI {
  ...
  render() {
    ...
    if (state === "USERNAME_PROMPT") {
      ...
      }).join("")}${players.length === 0 ? '<div class="empty-players-v2"><div class="empty-icon">${icon("users")}</div><p>Waiting for players to join...</p><small>Share the code above</small></div>' : ''}${Array.from({length: Math.max(0, 8-players.length)}).map((_,i)=>`<div class="player-row-v2 empty-slot"><div class="player-avatar empty">?</div><div class="player-info-v2"><strong>Waiting for player...</strong><small>Share code ${lobby?.code || ''} to invite</small></div></div>`).join("")}</div></div></div><div class="lobby-v2-side"><div class="lobby-settings-v2"><h3>${icon("settings")} LOBBY SETTINGS</h3><div class="setting-row-v2"><div><span>MODE</span><strong>${lobby?.mode?.toUpperCase() || 'CLASSIC'}</strong></div>${isHost ? `<select id="lobby-mode" data-setting="lobby-mode" class="mini-select"><option value="classic" ${lobby?.mode === 'classic' ? 'selected' : ''}>CLASSIC</option><option value="kart" ${lobby?.mode === 'kart' ? 'selected' : ''}>KART</option></select>` : ''}</div><div class="setting-row-v2"><div><span>MAP</span><strong>${ALL_TRACKS.find(t => t.id === lobby?.map_id)?.name || lobby?.map_id || 'Coast'}</strong></div>${isHost ? `<button data-action="change-map" class="chip small">CHANGE</button>` : ''}</div><div class="setting-row-v2 no-border"><div><span>MAX</span><strong>8 HUMAN • NO AI</strong></div><small class="no-ai-badge">NEVER BOTS</small></div></div><div class="lobby-my-selection-v2"><h3>${icon("car")} YOUR RIDE</h3><div class="selection-row-v2"><label>Car</label><select id="mp-car" data-custom="mp-car" class="full-select">${CARS.map(c => `<option value="${c.id}" ${s.car === c.id ? 'selected' : ''}>${c.name} — ${c.type}</option>`).join("")}</select></div>${lobby?.mode === 'kart' ? `<div class="selection-row-v2"><label>Character</label><select id="mp-character" data-custom="mp-character" class="full-select">${CHARACTERS.map(ch => `<option value="${ch.id}" ${s.character === ch.id ? 'selected' : ''}>${ch.name} — ${ch.trait}</option>`).join("")}</select></div>` : ''}<button class="primary wide ready-btn ${players.find(p => p.player_id === currentUserId)?.is_ready ? 'is-ready' : ''}" data-action="toggle-ready" id="ready-button"><span>${players.find(p => p.player_id === currentUserId)?.is_ready ? 'YOU ARE READY — CLICK TO UNREADY' : 'READY UP'}</span> ${icon("check")}</button><small class="ready-hint">Select car${lobby?.mode === 'kart' ? ' & character' : ''} then ready up. All players must be ready.</small></div><div class="lobby-actions-v2"><button class="primary wide start-race-btn ${canStart ? 'can-start' : ''}" data-action="start-multiplayer-race" ${!canStart ? 'disabled' : ''}><span>${!isHost ? 'WAITING FOR HOST TO START' : players.length < 2 ? `NEED ${2-players.length} MORE PLAYER${2-players.length>1?'S':''}` : !players.every(p => p.is_ready) ? 'WAITING FOR ALL READY' : `START RACE • ${players.length} PLAYERS`}</span> ${icon("flag")}</button><div class="lobby-action-hints">${isHost ? `<small>${icon("check")} You are host • You can start when all ready • Min 2, max 8, no AI</small>` : `<small>${icon("clock")} Select your ride and ready up. Host will start the race when everyone is ready.</small>`}</div></div></div></div></div></main>${this.footer()}${debugOverlay}`;
    } else if (state === "PLAYER_INTRO") {
      const players = g.lobbyPlayers || [];
      const current = players[g.playerIntroIndex];
      const username = current?.profiles?.username || current?.username || `Player${g.playerIntroIndex+1}`;
      const car = CARS.find(c => c.id === current?.selected_car) || CARS[0];
      const character = CHARACTERS.find(c => c.id === current?.selected_character);
      
      this.root.innerHTML = `<div class="player-intro-v2"><div class="intro-v2-top"><div class="intro-count-v2"><span>PLAYER</span><strong>${g.playerIntroIndex+1} / ${players.length}</strong></div><div class="progress-bar-v2"><i style="width:${((g.playerIntroIndex+1)/players.length)*100}%"></i></div></div><div class="intro-content-v2"><div class="intro-avatar-big">${safe(username).charAt(0).toUpperCase()}</div><div class="eyebrow">RACER ${g.playerIntroIndex+1} OF ${players.length} • HUMAN ONLY</div><h1>${safe(username).toUpperCase()}</h1><div class="intro-cards"><div class="intro-card"><span>${icon("car")} CAR</span><strong>${car.name}</strong><small>${car.type} • ${car.year} • ${car.speed} SPD / ${car.handling} HND</small></div>${character ? `<div class="intro-card"><span>${icon("users")} CHARACTER</span><strong>${character.name}</strong><small>${character.trait} • ${character.weight}kg</small></div>` : ''}<div class="intro-card highlight"><span>${icon("trophy")} READY TO RACE</span><strong>P${g.playerIntroIndex+1} ON GRID</strong><small>No AI — pure human competition</small></div></div></div><div class="intro-progress-v2"><button class="primary large" data-action="next-intro"><span>${g.playerIntroIndex+1 < players.length ? `NEXT: ${safe(players[g.playerIntroIndex+1]?.profiles?.username || players[g.playerIntroIndex+1]?.username || 'NEXT RACER')}` : 'SHOW TRACK'}</span> ${icon("arrow")}</button><small>${g.playerIntroIndex+1} of ${players.length} real players • No bots</small></div></div>`;
    } else if (state === "MAP_INTRO") {
      const lobby = g.lobby;
      const trackData = ALL_TRACKS.find(t => t.id === lobby?.map_id) || ALL_TRACKS[0];
      
      this.root.innerHTML = `<div class="map-intro-v2"><div class="map-intro-header"><div class="eyebrow"><i class="status-dot"></i> NEXT DESTINATION • ${g.lobbyPlayers.length} HUMAN PLAYERS</div><h1>${trackData.name.toUpperCase()}</h1><p>${trackData.subtitle}</p></div><div class="map-intro-grid"><div class="map-preview-v2"><div class="track-art ${trackData.id} large"><span class="art-sun"></span><span class="art-mountain one"></span><span class="art-mountain two"></span><span class="art-road"></span></div><div class="map-details-v2"><div><span>LAPS</span><strong>3</strong></div><div><span>LENGTH</span><strong>${(g.track.length / 1000).toFixed(2)} KM</strong></div><div><span>PLAYERS</span><strong>${g.lobbyPlayers.length} HUMAN</strong><small>NO AI</small></div><div><span>MODE</span><strong>${lobby?.mode?.toUpperCase()}</strong></div></div></div><div class="map-highlights-v2"><h3>${icon("flag")} TRACK INTEL</h3><div class="intel-grid"><div class="intel-item"><span>SHORTCUTS</span><strong>${trackData.kart ? '2 narrow lanes' : 'None — pure racing line'}</strong></div><div class="intel-item"><span>HAZARD</span><strong>${trackData.hazard ? trackData.hazard.toUpperCase() : 'None — clean track'}</strong></div><div class="intel-item"><span>RAMPS</span><strong>${trackData.kart ? '2 boost ramps' : '0 — ground level'}</strong></div><div class="intel-item"><span>BOOST PADS</span><strong>${trackData.kart ? '3 speed pads' : '0 — nitro only'}</strong></div></div><div class="intel-note">${icon("check")} All ${g.lobbyPlayers.length} human players start together. No AI fill, no bots. First to finish wins.</div><div class="map-tips"><h4>RACE TIPS</h4><ul><li>${icon("bolt")} Drift to charge nitro</li><li>${icon("flag")} Hit all 12 checkpoints per lap</li><li>${icon("trophy")} Stay on track — off-road slows you</li></ul></div></div></div><div class="map-intro-actions"><button class="primary large start-race-final" data-action="start-race-from-intro"><span>START RACE • ${g.lobbyPlayers.length} PLAYERS • GO!</span> ${icon("flag")}</button><small>3 laps • ${g.lobbyPlayers.length} real racers • ${trackData.name} • Good luck</small></div></div>`;
  ...
  change(el) {
    ...
    if (el.dataset.setting) {
      ...
      if (k === 'mp-car') {
        ...
        g.store.save();
        import("../multiplayer/Lobby.js").then(({ lobbyManager }) => {
          lobbyManager.updatePlayerSelection({ car: carId });
        });
        ...
        g.store.save();
        import("../multiplayer/Lobby.js").then(({ lobbyManager }) => {
          lobbyManager.updatePlayerSelection({ character: charId });
        });
      } else {
